  ...
  return <box style={{ width: '100%', height: '100%', backgroundColor: C.bg, padding: 1, flexDirection: 'column', gap: 1 }}>
    ...
    <box style={{ flexDirection: 'row', gap: 1, flexGrow: 1 }}>
      <Panel title="CASE" style={{ width: compact ? 31 : 38 }}>
        ...
        <text fg={C.muted}>{query ? report?.mode === 'repository' ? 'repository inventory · OSV advisory discovery' : 'public advisory and repository evidence' : 'run: npm run tui -- "<github-url>"'}</text>
      ...
      <Panel title="REPORT" style={{ width: compact ? 34 : 44 }}>
        {!report ? <text fg={C.faint}>The report appears after collection and proof complete.</text> : <><text fg={recordingMode ? completeColor : quality.readyForRecording ? C.green : C.amber}>{recordingMode ? recordingReady ? 'RECORDING-READY' : 'REVIEW REQUIRED' : quality.readyForRecording ? 'RECORDING-READY' : 'REVIEW REQUIRED'}</text><text fg={C.muted}>{quality.reason || 'Evidence quality unavailable.'}</text>{report.mode === 'repository' && <text fg={C.text}>scan · {summary.packagesChecked || 0} packages · {summary.totalAdvisories || 0} advisories</text>}{recordingMode && blockers.length > 0 && <><text fg={C.amber}>GATE BLOCKERS</text>{blockers.slice(0, compact ? 3 : 6).map((blocker) => <text key={blocker} fg={C.amber}>· {blocker}</text>)}</>}<text fg={C.text}>{summary.reached || 0} reached</text><text fg={C.red}>{summary.declaredOnly || 0} declared only</text><text fg={C.green}>{summary.notAffected || 0} outside range</text><text fg={C.muted}>{summary.unknown || 0} unknown</text>{(report.crossRepositoryCorrelations || []).length > 0 && <><text fg={C.faint}>SHARED RESOLUTIONS</text>{(report.crossRepositoryCorrelations || []).slice(0, compact ? 2 : 4).map((correlation) => <text key={`${correlation.packageName}@${correlation.version}`} fg={C.blue}>{correlation.packageName}@{correlation.version} · {correlation.repositoryCount} repos</text>)}</>}<text fg={C.faint}>EVIDENCE PATHS</text>{(report.repositories || []).slice(0, compact ? 2 : 4).map((finding) => { const cited = (finding.proof || []).filter((step) => ['observed', 'validated'].includes(step.status) && step.source).length; const chain = finding.dependencyPath?.length > 1 ? finding.dependencyPath.map((item) => `${item.name}@${item.version}`).join(' -> ') : null; return <box key={`${finding.repository}:${finding.advisoryId || 'advisory'}`} style={{ flexDirection: 'column', gap: 0, paddingBottom: 1 }}><text fg={verdictColor(finding.verdict)}>{finding.verdict} · {finding.repository}</text><text fg={C.muted}>{chain || `${cited}/${finding.proof?.length || 0} cited proof hops`}</text>{finding.pathObservation && <text fg={C.blue}>origin · {finding.pathObservation.observedAt?.slice(0, 10) || 'undated'} · {finding.pathObservation.commit || 'commit unavailable'}</text>}{finding.codeOwners?.length > 0 && <text fg={C.muted}>owner · {finding.codeOwners.join(', ')}</text>}{finding.sourceImpact?.files?.length > 0 && <text fg={C.blue}>source cone · {finding.sourceImpact.sampledFileCount} files · {finding.sourceImpact.observedEdgeCount} local edges</text>}</box> })}<text fg={C.faint}>FIX CHECK</text>{(report.challenge || []).slice(0, compact ? 3 : 6).map((item, index) => { const finding = (report.repositories || []).find((candidate) => candidate.repository === item.repository); return <box key={`${item.repository}:${finding?.advisoryId || index}`} style={{ flexDirection: 'column', gap: 0, paddingBottom: 1 }}><text fg={item.status === 'FIX_SURVIVES' || item.status === 'ALREADY_SAFE' ? C.green : verdictColor(item.status)}>{item.repository}: {item.status}</text><text fg={C.muted}>{finding?.verdict || 'UNKNOWN'} → {item.proposedVersion ? `upgrade ${item.proposedVersion}` : 'no admissible fix'} → {item.status}</text></box>})}</>}
        <box style={{ flexGrow: 1 }} />
