  ...
  return <box style={{ width: '100%', height: '100%', backgroundColor: C.bg, padding: 1, flexDirection: 'column', gap: 1 }}>
    ...
    <box style={{ flexDirection: 'row', gap: 1, flexGrow: 1 }}>
      ...
      <Panel title="REPORT" style={{ width: compact ? 34 : 44 }}>
        {!report ? <text fg={C.faint}>The report appears after collection and proof complete.</text> : <><text fg={quality.readyForRecording ? C.green : C.amber}>{quality.readyForRecording ? 'RECORDING-READY' : 'REVIEW REQUIRED'}</text><text fg={C.muted}>{quality.reason || 'Evidence quality unavailable.'}</text><text fg={C.text}>{summary.reached || 0} reached</text><text fg={C.red}>{summary.declaredOnly || 0} declared only</text><text fg={C.green}>{summary.notAffected || 0} not affected</text><text fg={C.muted}>{summary.unknown || 0} unknown</text><text fg={C.faint}>EVIDENCE PATHS</text>{(report.repositories || []).slice(0, compact ? 2 : 4).map((finding) => { const cited = (finding.proof || []).filter((step) => ['observed', 'validated'].includes(step.status) && step.source).length; const chain = finding.dependencyPath?.length > 1 ? finding.dependencyPath.map((item) => `${item.name}@${item.version}`).join(' -> ') : null; return <box key={finding.repository} style={{ flexDirection: 'column', gap: 0, paddingBottom: 1 }}><text fg={verdictColor(finding.verdict)}>{finding.verdict} · {finding.repository}</text><text fg={C.muted}>{chain || `${cited}/${finding.proof?.length || 0} cited proof hops`}</text></box> })}<text fg={C.faint}>FIX PROOF</text>{(report.challenge || []).slice(0, compact ? 3 : 6).map((item) => <text key={item.repository} fg={item.status === 'FIX_SURVIVES' || item.status === 'ALREADY_SAFE' ? C.green : verdictColor(item.status)}>{item.repository}: {item.status}</text>)}</>}
        <box style={{ flexGrow: 1 }} />
