 ...
 */
export function buildEvidenceQuality({ status = 'unknown', collectors = [], repositories = [], noFindingsIsValid = false } = {}) {
  ...
  for (const collector of collectors || []) {
    if (collector.status && !['completed', 'not_requested', 'skipped'].includes(collector.status)) {
  ...
  const complete = status === 'completed'
    ...
    && (repositories.length > 0 || noFindingsIsValid)
  const qualityStatus = complete ? 'complete' : status === 'completed' ? 'review' : 'partial'
  const reason = complete
    ? noFindingsIsValid && repositories.length === 0
      ? 'All requested public sources completed; no affected advisory matched the recorded dependency inventory.'
    ...
    : status !== 'completed'
      ...
      : repositories.length === 0
        ? 'No repository finding was produced from the collected evidence.'
