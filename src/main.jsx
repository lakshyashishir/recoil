...
function CopyCaseHandoff({ report, findings, summary, historical = false }) {
  ...
  const label = status === 'copied' ? 'Copied handoff' : status === 'failed' ? 'Copy unavailable' : 'Copy agent handoff'
...
function IncidentResponseLoop({ finding, onInspect }) {
  if (!finding) return null
  const challenge = finding.challenge || null
  const importer = finding.imports?.[0]
  const owners = [...new Set((finding.imports || []).flatMap((item) => item.owners || []))]
  const command = packageFixCommand(finding, challenge)
  const issueUrl = githubIssueDraftUrl({ finding, challenge, scenarioId: finding.caseId })
  const fixVerified = challenge?.status === 'FIX_SURVIVES' || challenge?.status === 'ALREADY_SAFE'
...
function IncidentView({ report, workspace, scenarioId, onOpenCase }) {
  ...
  const focusedFinding = activeFinding || selected?.findings?.find((finding) => finding.verdict === 'REACHED') || selected?.findings?.[0]
  const activeReportFindings = report?.repositories || []
