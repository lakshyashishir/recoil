...
function IncidentRoutePreview({ findings = [], onSelect }) {
  ...
  return <div className="incident-routes">{visible.map((finding, index) => {
    ...
    return <button key={findingKey(finding, index)} type="button" onClick={() => onSelect(finding)}><span className="incident-route-repo">{finding.repositoryKey}</span><span className="incident-route-line">{parts.slice(0, 5).map((part, partIndex) => <span className="incident-route-part" key={`${part}-${partIndex}`}><i>{partIndex + 1}</i><strong>{shorten(routeDisplayPart(part, finding), 21)}</strong>{partIndex < Math.min(parts.length, 5) - 1 && <em>→</em>}</span>)}</span></button>
  })}</div>
}
